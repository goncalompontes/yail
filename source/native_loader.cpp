...
namespace yail::detail
{
...
#ifdef _WIN64
            "4C 8B DC 49 89 5B 08 49 89 6B 10 49 89 73 18 57 41 54 41 55 41 56 41 57 48 81 EC ? ? ? ? 48 8B 05 ? ? ? ? 48 33 C4 48 89 84 24 ? ? ? ?", // Windows 11 25H2
    ...
    std::expected<void*, std::string> find_rtl_insert_inverted_function_table()
    {
        constexpr std::array signatures = {
#ifdef _WIN64
            "48 8B C4 48 89 58 08 48 89 68 10 48 89 70 18 57 48 83 EC 20 48 83 60 20", // Windows 11 25H2
    ...
    std::expected<void*, std::string> find_rtl_remove_inverted_function_table()
    {
        constexpr std::array signatures = {
#ifdef _WIN64
            // Verified on Windows 11 24H2 (build 26100) via IDA Pro — ntdll + 0x33134.
    ...
    std::expected<void*, std::string> find_ldrp_release_tls_entry()
    {
        constexpr std::array signatures = {
#ifdef _WIN64
            // Verified on Windows 11 24H2 (build 26100) via IDA Pro — ntdll + 0xD73C8.
